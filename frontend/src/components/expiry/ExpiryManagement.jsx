...
import { getDaysUntilExpiry, getCategoryColor } from '../../data/mockData';
...
const ExpiryManagement = () => {
    ...
    const withExpiry = medicines.map((m) => ({ ...m, daysLeft: getDaysUntilExpiry(m.expiry_date) }));
            ...
            {loading ? (
                <div className="flex items-center justify-center h-64">
                    <div className="text-center">
                        ...
                        <p className="mt-4 text-gray-500">Loading expiry data...</p>
                    </div>
                </div>
            ) : (
                <>
                            ...
                            {filtered.map((med) => {
                                ...
                                return (
                                    <tr key={med.id} className={`transition-colors ${med.daysLeft <= 0 ? 'bg-red-50/30' : med.daysLeft <= 30 ? 'bg-orange-50/20' : 'hover:bg-gray-50'}`}>
                                        ...
                                        <td className="px-5 py-3.5"><p className="text-sm font-semibold text-gray-900">{med.name}</p><p className="text-xs text-gray-400">{med.generic_name}</p></td>
                                        <td className="px-5 py-3.5"><span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${catColor.bg} ${catColor.text}`}>{med.category_name}</span></td>
                                        <td className="px-5 py-3.5 text-sm font-mono text-gray-500">{med.batch_number}</td>
                                        <td className="px-5 py-3.5 text-sm font-bold text-gray-700">{med.stock}</td>
                                        <td className="px-5 py-3.5 text-sm text-gray-600">{med.expiry_date}</td>
