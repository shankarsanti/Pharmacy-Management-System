...
const Sales = () => {
    ...
    const filtered = useMemo(() => {
        ...
        if (dateFilter === 'today') result = result.filter((s) => s.sale_date === today);
        else if (dateFilter === 'yesterday') result = result.filter((s) => s.sale_date === yesterday);

        // Payment filter
        if (paymentFilter) result = result.filter((s) => s.payment_method === paymentFilter);
        ...
        if (searchTerm) {
            ...
            result = result.filter(
                (s) =>
                    ...
                    s.billed_by_name.toLowerCase().includes(term) ||
                    s.items?.some((it) => it.medicine_name.toLowerCase().includes(term))
            );
        }
    ...
    const totalRevenue = filtered.reduce((s, x) => s + parseFloat(x.total), 0);
    const totalItems = filtered.reduce((s, x) => s + (x.items?.reduce((a, i) => a + i.quantity, 0) || 0), 0);
    ...
    const handlePrintAll = () => {
        const data = filtered;
        const totRevenue = data.reduce((s, x) => s + parseFloat(x.total), 0);
        const totItems = data.reduce((s, x) => s + (x.items?.reduce((a, i) => a + i.quantity, 0) || 0), 0);
        const totTax = data.reduce((s, x) => s + parseFloat(x.tax), 0);
        const totDiscount = data.reduce((s, x) => s + parseFloat(x.discount || 0), 0);

        // Payment summary
        const payMap = {};
        data.forEach((s) => {
            if (!payMap[s.payment_method]) payMap[s.payment_method] = { count: 0, amount: 0 };
            payMap[s.payment_method].count++;
            payMap[s.payment_method].amount += parseFloat(s.total);
        });
        ...
        const billMap = {};
        data.forEach((s) => {
            if (!billMap[s.billed_by_name]) billMap[s.billed_by_name] = { count: 0, amount: 0 };
            billMap[s.billed_by_name].count++;
            billMap[s.billed_by_name].amount += parseFloat(s.total);
        });
        ...
        const saleRows = data.map((s) => `<tr>
            ...
            <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9">${s.sale_date}</td>
            <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9">${new Date(s.created_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</td>
            <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9">${s.customer_name}</td>
            <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9">${s.items?.length || 0} items</td>
            <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9;font-weight:700">₹${parseFloat(s.total).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
            <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9">${s.payment_method}</td>
            <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9;font-size:11px;color:#64748b">${s.billed_by_name}</td>
        </tr>`).join('');
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {paginated.map((sale) => (
                                <tr key={sale.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setDetailSale(sale)}>
                                    ...
                                    <td className="px-5 py-3.5 text-sm text-gray-600">{sale.sale_date}</td>
                                    <td className="px-5 py-3.5 text-sm text-gray-500">{new Date(sale.created_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</td>
                                    <td className="px-5 py-3.5 text-sm font-medium text-gray-900">{sale.customer_name}</td>
                                    <td className="px-5 py-3.5 text-sm text-gray-600">{sale.items?.length || 0}</td>
                                    <td className="px-5 py-3.5 text-sm font-bold text-gray-900">₹{parseFloat(sale.total).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                    <td className="px-5 py-3.5"><span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${paymentStyles[sale.payment_method]}`}>{sale.payment_method}</span></td>
                                    <td className="px-5 py-3.5 text-sm text-gray-500">{sale.billed_by_name}</td>
                                </tr>
                            ))}
                            {paginated.length === 0 && (
                                <tr>
                ...
                {detailSale && (
                    <div>
                        <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                            <div><span className="text-gray-400">Date:</span> <span className="font-medium">{detailSale.sale_date}</span></div>
                            <div><span className="text-gray-400">Time:</span> <span className="font-medium">{new Date(detailSale.created_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</span></div>
                            <div><span className="text-gray-400">Customer:</span> <span className="font-medium">{detailSale.customer_name}</span></div>
                            <div><span className="text-gray-400">Payment:</span> <span className={`font-semibold px-2 py-0.5 rounded-lg ${paymentStyles[detailSale.payment_method]}`}>{detailSale.payment_method}</span></div>
                        </div>
                        <table className="w-full text-sm mb-4">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {detailSale.items?.map((item, i) => (
                                    <tr key={i}><td className="py-2 font-medium text-gray-900">{item.medicine_name}</td><td className="py-2 text-center">{item.quantity}</td><td className="py-2 text-right">₹{parseFloat(item.price).toFixed(2)}</td><td className="py-2 text-right font-semibold">₹{(item.quantity * parseFloat(item.price)).toFixed(2)}</td></tr>
                                ))}
                            </tbody>
                        </table>
                        <div className="border-t border-gray-200 pt-3 space-y-1 text-sm">
                            <div className="flex justify-between text-gray-500"><span>Subtotal</span><span>₹{parseFloat(detailSale.subtotal).toFixed(2)}</span></div>
                            <div className="flex justify-between text-gray-500"><span>Tax</span><span>₹{parseFloat(detailSale.tax).toFixed(2)}</span></div>
                            {parseFloat(detailSale.discount) > 0 && <div className="flex justify-between text-green-600"><span>Discount</span><span>-₹{parseFloat(detailSale.discount).toFixed(2)}</span></div>}
                            <div className="flex justify-between font-bold text-lg text-gray-900 pt-2 border-t"><span>Total</span><span>₹{parseFloat(detailSale.total).toFixed(2)}</span></div>
                        </div>
                    </div>
                )}
            </Modal>
