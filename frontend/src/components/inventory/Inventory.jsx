...
const Inventory = () => {
    ...
    const filtered = medicines.filter((m) => {
        const name = m.name || '';
        const generic = m.generic_name || m.generic || '';
        const category = m.category_name || m.category || '';
        const matchSearch = name.toLowerCase().includes(searchTerm.toLowerCase()) || generic.toLowerCase().includes(searchTerm.toLowerCase());
        const matchCategory = !categoryFilter || category === categoryFilter;
        const threshold = m.low_stock_threshold || m.lowStockThreshold || 10;
        const matchStock = !stockFilter ||
            (stockFilter === 'out' && m.stock === 0) ||
            (stockFilter === 'low' && m.stock > 0 && m.stock <= threshold) ||
            (stockFilter === 'ok' && m.stock > threshold);
    ...
    const openEdit = (med) => {
        setFormData({
            name: med.name,
            generic: med.generic_name || med.generic,
            category: med.category_name || med.category,
            batch: med.batch_number || med.batch,
            supplier: med.supplier_id || med.supplier,
            purchasePrice: med.purchase_price || med.purchasePrice,
            sellingPrice: med.selling_price || med.sellingPrice,
            stock: med.stock,
            expiry: med.expiry_date || med.expiry,
            medicineType: med.medicine_type || med.medicineType || 'Tablet',
            tabletsPerStrip: med.tablets_per_strip || med.tabletsPerStrip || 10,
            stripPrice: med.strip_price || med.stripPrice || med.selling_price || med.sellingPrice,
            looseTabletPrice: med.loose_tablet_price || med.looseTabletPrice || '',
            allowLooseSale: med.allow_loose_sale !== undefined ? med.allow_loose_sale : (med.allowLooseSale !== undefined ? med.allowLooseSale : true),
            lowStockThreshold: med.low_stock_threshold || med.lowStockThreshold || 20,
        });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const medData = {
                name: formData.name,
                generic_name: formData.generic,
                category_id: category?.id || null,
                batch_number: formData.batch,
                supplier_id: formData.supplier,
                purchase_price: +formData.purchasePrice || 0,
                selling_price: +formData.stripPrice || +formData.sellingPrice || 0,
                stock: +formData.stock || 0,
                expiry_date: formData.expiry || null,
                added_date: new Date().toISOString().split('T')[0],
                medicine_type: formData.medicineType,
                tablets_per_strip: +formData.tabletsPerStrip || 10,
                strip_price: +formData.stripPrice || +formData.sellingPrice || 0,
                loose_tablet_price: +formData.looseTabletPrice || 0,
                allow_loose_sale: formData.allowLooseSale,
                low_stock_threshold: +formData.lowStockThreshold || 20,
            };

            if (editingMed) {
                await medicinesAPI.update(editingMed.id, medData);
                toast.success(`${formData.name} updated successfully!`);
            } else {
                await medicinesAPI.create(medData);
    ...
    const handleDelete = async () => {
        try {
            await medicinesAPI.delete(deleteTarget.id);
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {paginated.map((med) => {
                                const category = med.category_name || med.category;
                                const catColor = getCategoryColor(category);
                                const expiry = med.expiry_date || med.expiry;
                                const daysLeft = getDaysUntilExpiry(expiry);
                                const medicineType = med.medicine_type || med.medicineType || 'Tablet';
                                const tabletsPerStrip = med.tablets_per_strip || med.tabletsPerStrip || 10;
                                const stripPrice = med.strip_price || med.stripPrice || med.selling_price || med.sellingPrice;
                                const looseTabletPrice = med.loose_tablet_price || med.looseTabletPrice;
                                const allowLooseSale = med.allow_loose_sale !== undefined ? med.allow_loose_sale : med.allowLooseSale;
                                const genericName = med.generic_name || med.generic;
                                return (
                                    <tr key={med.id} className="hover:bg-blue-50/30 transition-colors group">
                                        ...
                                        <td className="px-5 py-3.5">
                                            ...
                                            <div className="flex items-center gap-2 mt-0.5">
                                                <p className="text-xs text-gray-400">{genericName}</p>
                                                {allowLooseSale && medicineType === 'Tablet' && (
                                                    <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-violet-100 text-violet-600">Loose Sale</span>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-5 py-3.5">
                                            <span className={`px-2 py-0.5 rounded-lg text-xs font-semibold ${medicineType === 'Tablet' ? 'bg-cyan-50 text-cyan-700' : medicineType === 'Syrup' ? 'bg-pink-50 text-pink-700' : 'bg-gray-100 text-gray-600'}`}>
                                                {medicineType}
                                            </span>
                                            {medicineType === 'Tablet' && tabletsPerStrip > 1 && (
                                                <p className="text-[10px] text-gray-400 mt-0.5">{tabletsPerStrip}/strip</p>
                                            )}
                                        </td>
                                        <td className="px-5 py-3.5"><span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${catColor.bg} ${catColor.text}`}>{category}</span></td>
                                        <td className="px-5 py-3.5">{getStockBadge(med)}</td>
                                        <td className="px-5 py-3.5">
                                            <p className="text-sm font-medium text-gray-700">₹{stripPrice}</p>
                                            {allowLooseSale && looseTabletPrice > 0 && (
                                                <p className="text-[10px] text-gray-400">₹{looseTabletPrice}/tablet</p>
                                            )}
                                        </td>
                                        <td className="px-5 py-3.5">
                                            <span className={`text-sm ${daysLeft <= 30 ? 'text-red-600 font-bold' : daysLeft <= 90 ? 'text-amber-600' : 'text-gray-500'}`}>{expiry}</span>
                                        </td>
                                        <td className="px-5 py-3.5">
