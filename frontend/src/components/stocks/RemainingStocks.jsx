...
const RemainingStocks = () => {
    ...
    const totalStockValue = medicines.reduce((sum, m) => sum + m.stock * m.selling_price, 0);
    ...
    const inStockCount = medicines.filter((m) => m.stock > (m.low_stock_threshold || 10)).length;
    const totalPurchaseValue = medicines.reduce((sum, m) => sum + m.stock * m.purchase_price, 0);
    ...
    const filtered = useMemo(() => {
        ...
        else if (stockFilter === 'low') result = result.filter((m) => m.stock > 0 && m.stock <= (m.low_stock_threshold || 10));
        else if (stockFilter === 'medium') result = result.filter((m) => m.stock > (m.low_stock_threshold || 10) && m.stock <= 50);
        ...
        result.sort((a, b) => {
            ...
            switch (sortBy) {
                ...
                case 'stock':
                    ...
                    break;
                case 'value':
                    comparison = a.stock * a.selling_price - b.stock * b.selling_price;
                    break;
                case 'expiry':
    ...
    const categoryStockSummary = useMemo(() => {
        const summary = {};
        medicines.forEach((m) => {
            if (!summary[m.category_name]) summary[m.category_name] = { totalStock: 0, totalValue: 0, count: 0 };
            summary[m.category_name].totalStock += m.stock;
            summary[m.category_name].totalValue += m.stock * m.selling_price;
            summary[m.category_name].count += 1;
        });
    ...
    const handlePrintAll = () => {
        ...
        const catMap = {};
        data.forEach((m) => {
            if (!catMap[m.category_name]) catMap[m.category_name] = { count: 0, stock: 0, value: 0 };
            catMap[m.category_name].count++;
            catMap[m.category_name].stock += m.stock;
            catMap[m.category_name].value += m.stock * m.selling_price;
        });
        ...
        const getStatusLabel = (m) => {
            ...
            if (m.stock <= (m.low_stock_threshold || 10)) return '<span style="background:#fef3c7;color:#b45309;padding:2px 8px;border-radius:6px;font-size:10px;font-weight:700">Low Stock</span>';
        ...
        const medRows = data.map((m) => {
            const isTab = m.medicine_type === 'Tablet' && m.tablets_per_strip > 1;
            const equiv = isTab && m.stock > 0 ? `${Math.floor(m.stock / m.tablets_per_strip)}s + ${m.stock % m.tablets_per_strip}t` : '—';
            const dLeft = getDaysUntilExpiry(m.expiry_date);
            return `<tr>
                <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9;font-size:11px;color:#94a3b8">${m.id}</td>
                <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9"><strong>${m.name}</strong><br><span style="font-size:10px;color:#94a3b8">${m.generic_name}</span></td>
                <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9">${m.category_name}</td>
                ...
                <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9;font-weight:700">₹${(m.stock * m.selling_price).toLocaleString('en-IN')}</td>
                <td style="padding:7px 10px;border-bottom:1px solid #f1f5f9;${dLeft <= 30 ? 'color:#dc2626;font-weight:700' : ''}">${m.expiry_date}</td>
            </tr>`;
        }).join('');
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {paginated.map((med) => {
                                    const stockInfo = getStockLevel(med.stock, med.low_stock_threshold);
                                    const catColor = getCategoryColor(med.category_name);
                                    const daysLeft = getDaysUntilExpiry(med.expiry_date);
                                    const isTablet = med.medicine_type === 'Tablet' && med.tablets_per_strip > 1;
                                    const stockDisplay = isTablet ? getStockDisplay(med.stock, med.tablets_per_strip) : null;
                                        ...
                                        <tr key={med.id} className="hover:bg-blue-50/30 transition-colors group">
                                            ...
                                            <td className="px-5 py-3.5">
                                                ...
                                                <div className="flex items-center gap-2 mt-0.5">
                                                    ...
                                                    {med.allow_loose_sale && isTablet && (
                                                        <span className="px-1 py-0.5 rounded text-[8px] font-bold bg-violet-100 text-violet-600">LOOSE OK</span>
                                                    )}
                                                </div>
                                            </td>
                                            <td className="px-5 py-3.5">
                                                <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${catColor.bg} ${catColor.text}`}>{med.category_name}</span>
                                            </td>
                                            <td className="px-5 py-3.5">
                                            ...
                                            <td className="px-5 py-3.5">
                                                {isTablet && med.stock > 0 ? (
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] text-gray-400">{med.tablets_per_strip} tablets/strip</p>
                                                    </div>
                                                ) : (
                                            ...
                                            <td className="px-5 py-3.5">
                                                <p className="text-sm font-bold text-gray-900">₹{(med.stock * med.selling_price).toLocaleString()}</p>
                                                <p className="text-xs text-gray-400">@ ₹{med.selling_price}/strip</p>
                                            </td>
                                            <td className="px-5 py-3.5">
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-start justify-between mb-3">
                                        ...
                                        <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold ${catColor.bg} ${catColor.text}`}>{med.category_name}</span>
                                    </div>
                                    <div className="mb-3">
                                        <div className="flex items-center justify-between mb-1">
                                    ...
                                    <div className="flex items-center justify-between text-xs text-gray-400">
                                        <span>₹{(med.stock * med.selling_price).toLocaleString()}</span>
                                        <span className={daysLeft <= 30 ? 'text-red-500 font-bold' : ''}>
                                            {daysLeft <= 0 ? 'Expired' : `Exp: ${med.expiry_date}`}
                                        </span>
                                    </div>
                                </div>
                            );
            ...
            <Modal isOpen={!!viewMed} onClose={() => setViewMed(null)} title="Medicine Stock Details" size="md">
                {viewMed && (() => {
                    const stockInfo = getStockLevel(viewMed.stock, viewMed.low_stock_threshold);
                    const catColor = getCategoryColor(viewMed.category_name);
                    const daysLeft = getDaysUntilExpiry(viewMed.expiry_date);
                    const isTablet = viewMed.medicine_type === 'Tablet' && viewMed.tablets_per_strip > 1;
                    const stockDisplay = isTablet ? getStockDisplay(viewMed.stock, viewMed.tablets_per_strip) : null;
                        ...
                        <div className="space-y-5">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-sm text-gray-400">{viewMed.generic_name}</p>
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className={`inline-block px-2.5 py-0.5 rounded-lg text-xs font-semibold ${catColor.bg} ${catColor.text}`}>{viewMed.category_name}</span>
                                        <span className={`px-2 py-0.5 rounded-lg text-xs font-semibold ${viewMed.medicine_type === 'Tablet' ? 'bg-cyan-50 text-cyan-700' : 'bg-gray-100 text-gray-600'}`}>{viewMed.medicine_type}</span>
                                        {viewMed.allow_loose_sale && isTablet && (
                            ...
                            <div className={`rounded-xl p-4 ${stockInfo.bgColor} border`}>
                                ...
                                {isTablet && viewMed.stock > 0 && (
                                    <div className="mt-3 flex items-center gap-2 bg-white/40 rounded-lg p-2">
                                        ...
                                        <span className="text-[10px] text-gray-400">({viewMed.tablets_per_strip} tablets/strip)</span>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {[
                                    ...
                                    { label: 'Batch Number', value: viewMed.batch_number },
                                    { label: 'Medicine Type', value: viewMed.medicine_type },
                                    { label: 'Tablets Per Strip', value: isTablet ? viewMed.tablets_per_strip : '—' },
                                    { label: 'Strip Price', value: `₹${viewMed.strip_price || viewMed.selling_price}` },
                                    { label: 'Loose Tablet Price', value: viewMed.allow_loose_sale && viewMed.loose_tablet_price ? `₹${viewMed.loose_tablet_price}` : 'N/A' },
                                    { label: 'Purchase Price', value: `₹${viewMed.purchase_price}` },
                                    { label: 'Low Stock Threshold', value: `${viewMed.low_stock_threshold || 10} tablets` },
                                    { label: 'Total Stock Value', value: `₹${(viewMed.stock * viewMed.selling_price).toLocaleString()}` },
                                    { label: 'Profit per Strip', value: `₹${(viewMed.strip_price || viewMed.selling_price) - viewMed.purchase_price}` },
                                    { label: 'Expiry Date', value: viewMed.expiry_date, highlight: daysLeft <= 30 },
