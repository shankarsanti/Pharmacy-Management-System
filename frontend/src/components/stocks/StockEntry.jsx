...
import { medicinesAPI, suppliersAPI, stockEntriesAPI } from '../../services/api';
...
const StockEntry = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [entriesRes, medicinesRes, suppliersRes] = await Promise.all([
            ...
            const transformedEntries = entriesRes.data.stockEntries.map(entry => ({
                ...
                items: entry.items.map(item => ({
                    ...
                    stripsPurchased: item.strips_purchased,
                    looseTabletsPurchased: item.loose_tablets_purchased,
                    tabletsPerStrip: item.tablets_per_strip,
                    qty: item.quantity,
    ...
    const handleItemChange = (index, field, value) => {
        ...
        if (field === 'medId' && value) {
            ...
            if (med) {
                ...
                updatedItems[index].purchasePrice = med.purchase_price;
                updatedItems[index].sellingPrice = med.selling_price;
                updatedItems[index].batch = med.batch_number;
                updatedItems[index].tabletsPerStrip = med.tablets_per_strip || 10;
            }
        }
    ...
    const handleSave = async () => {
        ...
        try {
            const supplierInfo = suppliers.find((s) => s.id === formData.supplier);
            ...
            const stockEntryData = {
                ...
                items: formData.items.map((it) => ({
                    medicine_id: it.medId,
                    batch_number: it.batch,
                    strips_purchased: Number(it.stripsPurchased) || 0,
                    loose_tablets_purchased: Number(it.looseTabletsPurchased) || 0,
                    tablets_per_strip: Number(it.tabletsPerStrip) || 10,
                    quantity: Number(it.qty),
                    purchase_price: Number(it.purchasePrice),
            ...
            const response = await stockEntriesAPI.create(stockEntryData);
            ...
            toast.error(error.response?.data?.message || 'Failed to save stock entry');
        }
    };

    const handleDelete = async () => {
        try {
            await stockEntriesAPI.delete(deleteTarget.id);
            ...
            {loading ? (
                <div className="flex items-center justify-center h-64">
                    <div className="text-center">
                        ...
                        <p className="mt-4 text-gray-500">Loading stock entries...</p>
                    </div>
                </div>
            ) : (
                <>
